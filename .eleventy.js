...
const filterGetCategoryOptions = require("./src/config/filters/getCategoryOptions");
const filterHasCategory = require("./src/config/filters/hasCategory");
const filterShareCategory = require("./src/config/filters/shareCategory");
const filterFormatCategory = require("./src/config/filters/formatCategory");
...
module.exports = function (eleventyConfig) {
    ...
    /*
     * 🚗 Get Category Options Filter
     * Gets unique transmission and seat options for a category
     * Usage: {{ collections.car | getCategoryOptions("Category Name") }}
     */
    eleventyConfig.addFilter("getCategoryOptions", filterGetCategoryOptions);

    /*
     * 🚗 Has Category Filter
     * Checks if a car belongs to a specific category (handles both array and string formats)
     * Usage: {{ car.data.category | hasCategory("Category Name") }}
     */
    eleventyConfig.addFilter("hasCategory", filterHasCategory);

    /*
     * 🚗 Share Category Filter
     * Checks if two cars share at least one category (handles both array and string formats)
     * Usage: {{ car1.data.category | shareCategory(car2.data.category) }}
     */
    eleventyConfig.addFilter("shareCategory", filterShareCategory);

    /*
     * 🚗 Format Category Filter
     * Formats category field for display (handles both array and string formats)
     * Usage: {{ car.data.category | formatCategory }}
     */
    eleventyConfig.addFilter("formatCategory", filterFormatCategory);

    /*
     * 📝 Markdown Filter
     * Converts markdown strings to HTML
     * Usage: {{ markdownString | markdown | safe }}
     */
